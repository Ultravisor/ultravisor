...
defmodule Ultravisor.DbHandler do
  ...
  @spec start_link(map()) :: {:ok, pid(), term()} | :gen_statem.start_ret()
  def start_link(config),
    do: :proc_lib.start_link(__MODULE__, :init, [config])
  ...
  def handle_event(:info, {proto, _, bin}, :authentication, data(id: id) = data)
    ...
    case resp do
      ...
      {:ready_for_query, acc} ->
        ...
        :proc_lib.init_ack({:ok, self(), sock})
  ...
  def handle_event(_, {:EXIT, pid, reason}, state, data(mode: mode, sock: sock)) do
    if reason != :normal do
      Logger.error(
