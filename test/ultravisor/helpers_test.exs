  ...
  describe "valid_name?/1" do
    ...
    property "printable strings with at most 63 *bytes* are valid" do
      check all(name <- string(:printable, min_length: 1, max_length: 63)) do
        ...
        assert @subject.valid_name?(name) == byte_size(name) < 64
      end
    end

    property "non-printable strings are invalid" do
      check all(prefix <- string(:utf8), suffix <- string(:utf8)) do
        refute @subject.valid_name?(prefix <> <<0>>)
        refute @subject.valid_name?(<<0>> <> suffix)
        refute @subject.valid_name?(prefix <> <<0>> <> suffix)

        refute @subject.valid_name?(prefix <> <<0x10>>)
        refute @subject.valid_name?(<<0x10>> <> suffix)
        refute @subject.valid_name?(prefix <> <<0x10>> <> suffix)
      end
    end
  end
end
